...
```{r}
  ...
  if (!"rep_num" %in% names(obs)) {
    ...
    ggplot(data=pred_lines, aes(color=as.factor(condition_id))) +
      ...
      labs(title = category, y = "Mean proportion", x = "Repetition")+coord_cartesian(ylim=c(0,.6))+theme(legend.position = "none")+facet_wrap(~(as.numeric(condition_id)%/%3))
  }
