...
## Core tables
The core information in our dataset is stored in three tables: Trials, Messages, and Choices. Thethe key unit in our schema is the trial, which we define as the actions associated with picking out one target image out of a pool of options. Each trial can be associated with messages -- speech acts or typed messages from describer and matchers, which are associated with the trial, who produced them, and what order they are in within the trial. Trials also have choice information -- who selected what option. 

Trials contain information such as what game they were in, who the describer and matchers were, what the image set was, and when in the game they were, and what condition they were part of. We also label trials based on whether they were "ordering" or "matching" trials.  TODO clarify!

We include dataset_id in every table for easier filtering

### Trials
Each row in the Trials table contains information about a single trial. Each trial has a unique ID used to cross-link with other tables. Each trial has information on what was happening that trial and how it fits into the larger structure of a game. 

Each trial is associated with a describer and a list of matchers via their player ids. Each trial has a target and a set of image options, linked via their image ids. 

Each trial is labelled as either an "order" trial or a "match trial". In matching trials, a given target is specifically highlighted and the matcher picks one and then moves on. In ordering, the goal is to reproduce a given order of the set (or subset) of targets. We separate these into trials, but there is flexibility in that a matcher might revise their choice to one trial after later.

The basic structure of an iterated reference game is a series of trials (acts of picking out different targets) that are nested within repetitions through the target set (sometimes called blocks or rounds). We number trials starting from 1 across reps and reps starting from 1. 

Some game designs have more complications. 
There is not a separate game table, but each trial is nested within a game. Each game is a set of people who play with each other, possibly on a network of interlocking pairs or with substitutions (see metadata). Following @hawkins whatever, we include a room number for when there are two trials going on in parallel in the same game (this occurs on network games where two pairs play and then swap). 

If there is a change in how the game is working, such as a swap in partners, or in the pool of stimuli, we define that as a new stage, recorded via the stage number. For instance, the network swap games have multiple stages, partner swap designs have multiple stages, and some games with entrainment and test phases are multi-stage and games with a separate "generalization" round have multiple stages. 

In general, restricting to "stage 1 data" (as is possible in the shiny app and via filtering downloaded data) allows for a more even comparison across games. 

While the trials table includes information about the set up of the trial, the actual content of trial consists of two kinds of actions: messages and choices. 

### Messages
Each message is one turn of dialogue or one message that was typed in a chat window, depending on the game modality. A message has text, the content of the message, and is associated with a player_id, who produced the message.  Messages are also associated with the player's role (either matcher or describer). All messages have a trial id (linking to the trials table), and Within a trial, messages are given a message number defining their sequential order. There is a field, message_irrelevant, to preserve labels about off-topic messages (for example chit chat or meta-discussion about strategy). There is also a time_stamp field for when the timing of a message within a trial is known. 
We do not remove off-topic messages, or trim messages with some extraneous content, but we do use spelling-corrected versions of written messages where available. 

### Choices
The other type of information is about what a player selects, which is recorded in a choices table. Each choice is tied to a given trial and a given player_id.  Each choice is associated with a selected target denoted with choice_id that matches the image_id (and can be compared with the trials target_id). There is again the option for a time-stamp to specify how long into the trial the selection was made. 

In order games, we only record the final choices players made for each position. 
...
## Metadata
In addition to the core information about what happened during the game, we have several tables for metadata about the players, images, and experimental features of the study. 

### Datasets
The datasets table contains only bibliographic information, linking our dataset_id to the full and short citations for that dataset. 

### conditions
Iterated reference games can vary along a number of design features, which we code for. Most of these factors are coded in the "conditions" table. 

We code several meta-data factors as condition information. These include

* language: the language in which the iterated reference game was conducted 

* group-size: the number of players in a game, where we define game as a set of players who interact with each other but no one else
...
### Players
The player table contains some basic demographic information about participants when available. It can be linked via the player ids. We track players age, gender, native language, race, and level of education. These are frequently not available in the datasets. 

### Images (& image files)
unlike the other tables that all contain dataset_id for easy filtering, images are often reused across different datasets. Each image has a unique image id for cross referencing with the trials and choices tables. We also record image_type (tangram, line drawing, fribble, etc). For tangram images that are in kilogram (CITE), we include the kilogram id, for cross-referencing. When we have access to the actual image file, we store those also in redivis, and include the path in image_path. 

## Derived
In addition to these tables, we also have a redivis workflow for creating derivative tables. These include vector embeddings and stanza parses of the messages data and also tables of summary statistics. 

as additional processing pipelines are developed, we may add more derived tables. 
...
## Processing 
We started developing refbank with the iterated reference games that were conducted by members of the team, or where the data was already posted publicly. We then contacted authors of a number of iterated reference game papers asking if they were willing to share their data. 

The raw data we recieved varied in format. In some cases, we recieved pre-cleaned tablular data. In other cases, we recieved output from experimental software. In some cases we recieved transcripts of spoken datasets, in other cases we got raw audio, which we transcribed using whisper (or whatever we actually used). 

We attempted to fill in missing information, for instance, to identity trial boundaries on transcribed audio, or identify which image went with which label. Each dataset has a readme with import notes, including when we had to do processing or inference. 
...
Metadata was coded from the data and from the paper or write-up. In a few cases, condition information was not fully specified in the paper. In these cases, we either talked with the authors, or made our best guesses from the paper. When there were borderline cases that were not explicit we inferred them and made a note in the readme. 
...
dataset_summary_stats <- condition_summary |> summarize(datasets=nrow(distinct(datasets)), 
                                                        words=sum(total_num_words),
                                                        trials=sum(total_num_trials),
                                                        games=sum(total_num_games),
                                                        players=sum(total_num_players))
```
Refbank currently has data from `r dataset_summary_stats$datasets[1] |> scales::comma()` datasets including `r dataset_summary_stats$words[1] |> scales::comma()` words from `r dataset_summary_stats$trials[1] |> scales::comma()` referential trials from `r dataset_summary_stats$games[1] |> scales::comma()` games and `r dataset_summary_stats$players[1] |> scales::comma()` participants (Table TODO).
...
Datasets vary in their size and design features. 
...
# removing Trials/game for now 
  data_features_all |>
   knitr::kable(
     ...
     escape=F,
     linesep = "",
     caption = "Design features of each dataset. 'ltd' is used as an abbrevation of limited. Multiple values indicate that conditions varied within the data",
