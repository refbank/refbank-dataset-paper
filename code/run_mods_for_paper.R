...
options(
  ...
  brms.cores = parallel::detectCores(),
  brms.iter = 500,
  brms.chains = 4,
...
# noun_mod <- brm(NOUN ~ rep_num + (rep_num | dataset_id / condition_id),
#   file = here("cached_model_files/mods/noun_mod.rds"),
#   prior = logistic_dv_priors,
#   data = per_describer_for_model |> filter(!is.na(NOUN)) |> filter(stage_num == 1)
# )
#
# verb_mod <- brm(VERB ~ rep_num + (rep_num | dataset_id / condition_id),
#   file = here("cached_model_files/mods/verb_mod.rds"),
#   prior = logistic_dv_priors,
#   data = per_describer_for_model |> filter(!is.na(VERB)) |> filter(stage_num == 1)
# )
#
# modifier_mod <- brm(MODIFIER ~ rep_num + (rep_num | dataset_id / condition_id),
#   file = here("cached_model_files/mods/modifier_mod.rds"),
#   prior = logistic_dv_priors,
#   data = per_describer_for_model |> filter(!is.na(MODIFIER)) |> filter(stage_num == 1)
# )
#
# function_mod <- brm(FUNCTION ~ rep_num + (rep_num | dataset_id / condition_id),
#   file = here("cached_model_files/mods/function_mod.rds"),
#   prior = logistic_dv_priors,
#   data = per_describer_for_model |> filter(!is.na(FUNCTION)) |> filter(stage_num == 1)
# )
#
