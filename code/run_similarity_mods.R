...
>>>>>>> switch to log_rep_num

to_next_mod <- ordbetareg(sim ~ log_rep_num + (log_rep_num || dataset_id / condition_id),
  manual_prior = sim_priors,
  file = here("cached_model_files/mods/to_next_mod.rds"),
  data = sims_for_model |> filter(sim_type == "to_next")
)

diverge_mod <- ordbetareg(sim ~ log_rep_num + (log_rep_num || dataset_id / condition_id),
  manual_prior = sim_priors,
