...
for_table |> ungroup() |> select(-dataset_id, -full_cite) |> View()
...
```{r}
 make_line_plot(
      df |> filter(rep_num<7, group_size>1, stage_num==1) |> mutate(modality=str_c("Modality: ", modality)), "words",
    "group_size", "modality", T, T,
...
# for poster 
 ...
 make_line_plot(
      df |> filter(rep_num<7, group_size>1, stage_num==1) |>  filter(modality!="oral-remote") |> mutate(modality=str_c("Modality: ", modality)), "words",
    "group_size", "modality", T, T,
      "Reduction: Total describer utterance length across repetitions",
      "Length (words)", c(0.85, 0.75)
    ) + 
...
ggsave("reduction-poster.png", width=7, height=3)
...
```{r}
make_line_plot(
      df |> filter(rep_num<7, stage_num==1) |> mutate(role_constancy=ifelse(role_constancy=="yes", "Constant describer", "Rotating describer")), "to_next",
      "option_size", "role_constancy", T, T,
...
ggsave("similarity.png", width=7, height=4)
...
make_line_plot(
  ...
  coord_cartesian(xlim=c(2,6))+
        theme(title=element_blank(), 
                                             ...
                                             axis.title=element_text(size=16),
                                             legend.position = "right",
                                                           legend.margin=margin(-2,-2,-2,-2))+  guides(color=guide_legend(ncol=1))


ggsave("similarity-poster.png", width=7, height=3)
...
library(ggthemes)
make_line_plot(
      for_plot |> filter(rep_num<7) |> filter(modality!="oral-remote") |>, "count",
...
ggsave("pos.png", height=3, width=7)
...
ggsave("pos-poster.png", height=3.5, width=7)

```

```{r}

for_plot_2 <- pos |> group_by(game_id, group_size, stage_num, dataset_id, condition_label, modality, option_size, rep_num) |> # update this set
  summarize(across(c(nouns, modifiers, verbs, pro, closed), sum),
            trials=n()) |> 
  mutate(total=nouns+modifiers+verbs+pro+closed,
         across(c(nouns, verbs, modifiers, pro, closed),\(c){c/nouns})) |> 
  select(-total) |> pivot_longer(nouns:closed, values_to = "count", names_to="type") |> 
  mutate(type=case_when(
    type=="closed"~"Other closed class",
    type=="modifiers"~"Modifiers",
...
```{r}
make_line_plot(
      for_plot_2 |> filter(rep_num<7), "count",
      "modality", "dataset_id", F, T,
      "TODO title",
      "Fraction words", c(0.85, 0.25))+
  facet_wrap(~type)+stat_summary(aes(group=interaction(dataset_id), weight=trials), geom="line", alpha=.3)+
